// src/components/HebrewDateConverter/hebrewDateUtils.js
...
export const getDateNote = (date) => {
  // Get Hebrew date parts for checking holidays
  ...
  if (hebrewMonth === 9 && hebrewDay === 25) return 'חנוכה (יום ראשון)';
 ...
 * @returns {Object} Hebrew date info including date string, day of week, and special note
 */
export const convertToHebrewDate = (date) => {
  ...
  // Get any special note for this date
  ...
  if (yearPart) {
    ...
    // Handle years based on their range
    ...
    } else if (yearNum >= 4000 && yearNum < 5000) {
      // 4000-5000 era
      const lastThreeDigits = yearNum % 1000;
      year = `ה׳ד׳${convertToGematriya(lastThreeDigits)}`;
    } else if (yearNum >= 3000 && yearNum < 4000) {
      // 3000-4000 era
      const lastThreeDigits = yearNum % 1000;
      year = `ה׳ג׳${convertToGematriya(lastThreeDigits)}`;
    } else {
      // Other years - just convert the whole number
      year = convertToGematriya(yearNum);
    }
  } else {
    // If yearPart is missing, try to calculate it
    const hebrewYearFormatter = new Intl.DateTimeFormat('he', { 
      calendar: 'hebrew', 
      year: 'numeric' 
    });
    
    const rawYearStr = hebrewYearFormatter.format(date);
    console.log("Raw Hebrew year:", rawYearStr);
    
    // Extract year number from the formatted string
    const yearMatch = rawYearStr.match(/\d+/);
    if (yearMatch) {
      const yearNum = parseInt(yearMatch[0], 10);
      // Apply the same logic based on year range
      if (yearNum >= 5000 && yearNum < 6000) {
        const lastTwoDigits = yearNum % 100;
        year = `ה׳תש${convertToGematriya(lastTwoDigits)}`;
      } else if (yearNum >= 4000 && yearNum < 5000) {
  ...
  if (day && month) {
    ...
    // If we have day and month but no year, try harder to get the year
    if (!year) {
      try {
        const hebrewYear = new Intl.DateTimeFormat('he', { 
          calendar: 'hebrew', 
          year: 'numeric' 
        }).format(date);
        
        const yearMatch = hebrewYear.match(/\d+/);
        if (yearMatch) {
          const yearNum = parseInt(yearMatch[0], 10);
          if (yearNum >= 5000 && yearNum < 6000) {
            const lastTwoDigits = yearNum % 100;
        ...
        console.error("Error getting Hebrew year:", e);
      }
    }
